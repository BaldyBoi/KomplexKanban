...
double arg(const Complex com) {
    ...
    if(imag(com) < 0 && real(com) < 0) {
        //arcTangent has a limited domain. adding pi will guarntee that it is within the domain.
        ret -= PI;
    }
    return ret;
}
...
/*
*returns complex number with a magnitude and angle
*ang defaults to 0
*/
...
/*
overrides == operator and checks equality between complex functions
*/
bool Complex::operator==(const Complex &rhs) const {
    //compares both parts of a complex number
    return re == rhs.re &&  im == rhs.im;
}
/*
overrides not equal operator to check equality in all aspects of complex number
*/
bool Complex::operator!=(const Complex &rhs) const {
    return !(rhs == *this);
}
